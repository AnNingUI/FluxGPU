 ...
 * 定义并注册一个 struct 类型
 */
export function defineStruct<T extends StructFields>(
	name: string,
	fields: T
): StructType<T> {
	...
	for (const field of layout.fields) {
		...
		if (typeof value === "number") {
			...
			for (let i = 0; i < value.length; i++) {
				if (
					wgslType.includes("f32") ||
					wgslType.startsWith("vec") ||
					wgslType.startsWith("mat")
				) {
...
export class StructVar<T extends StructType> extends StructExpr<T> {
	...
	$<K extends keyof T["__fields"] & string>(
		fieldName: K
	): FieldRefFor<T["__fields"][K]> {
		const fieldType = this.type.__fields[fieldName];
		return createFieldRef(
			`${this.name}.${fieldName}`,
			fieldType
		) as FieldRefFor<T["__fields"][K]>;
	}
}
...
type FieldRefFor<T extends WGSLType> = T extends VectorType
	? VecFieldRef<T>
...
function createFieldRef<T extends WGSLType>(
	code: string,
	type: T
): FieldRefFor<T> {
...
// 类型映射
type ElementRefFor<T extends WGSLType> = T extends StructType
	? StructElementRef<T>
	: ElementRef<T>;

// 工厂函数
function createElementRef<T extends WGSLType>(
	code: string,
	type: T
): ElementRefFor<T> {
	if (isStructType(type)) {
...
// 根据类型返回对应的变量类型
type VarFor<T extends WGSLType> = T extends VectorType
	? VecVar<T>
	: T extends StructType
	? StructVar<T>
	: T extends ArrayType
...
export class ShaderBuilder {
	...
	/** 声明 sampler 绑定 */
	sampler(name: string, group: number, binding: number): Expr<SamplerType> {
		this.bindings.push(
	...
	>(
		...
		for (const [name, v] of Object.entries(varyings) as [
			string,
			{ location: number; type: WGSLType }
		][]) {
			varyingFields.push(
				`  @location(${v.location}) ${name}: ${v.type.__wgslType},`
			);
		}
		...
		for (const [name, attr] of Object.entries(attributes) as [
			string,
			VertexAttribute
		][]) {
			params.push(
				`@location(${attr.location}) ${name}: ${attr.type.__wgslType}`
			);
		}
	...
	fragment<TInputs extends Record<string, FragmentInput>>(
		...
		if (hasInputs) {
			...
			for (const [name, inp] of Object.entries(inputConfig) as [
				string,
				FragmentInput
			][]) {
				this.emit(
					`  @location(${inp.location}) ${name}: ${inp.type.__wgslType},`
				);
			}
			this.emit(`}`);
			this.emit(``);
		...
		const fragmentBuiltins: FragmentBuiltins = {
			position: new VecExpr(
				vec4(f32),
				hasInputs ? "input.position" : "frag_position"
			),
		...
		for (const [name, inp] of Object.entries(inputConfig) as [
			...
			inputs[name] = this.createTypedVar(
				hasInputs ? `input.${name}` : name,
				inp.type
			);
		}
	...
	private createContext(): ShaderContext {
		...
		return {
			...
			for(init, condition, update, body): void {
				...
				self.emit(
					`for (var ${name}: ${type.__wgslType} = ${start}; ${condition(
						iExpr
					).toWGSL()}; ${update(iVar)}) {`
				);
				self.indent++;
...
export function makeVec2<T extends ScalarType>(
	...
	return new VecExpr(
		vec2(elementType),
		`vec2<${elementType.__wgslType}>(${xCode}, ${yCode})`
	);
}
...
export function makeVec3<T extends ScalarType>(
	...
	return new VecExpr(
		vec3(elementType),
		`vec3<${elementType.__wgslType}>(${xCode}, ${yCode}, ${zCode})`
	);
}
...
export function makeVec4<T extends ScalarType>(
	...
	return new VecExpr(
		vec4(elementType),
		`vec4<${elementType.__wgslType}>(${xCode}, ${yCode}, ${zCode}, ${wCode})`
	);
}
