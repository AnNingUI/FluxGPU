...
export function createSDFFragmentShader(): string {
	...
	builder.vertex(
		{
			attributes: {
				position: { location: 0, type: vec2(f32) },
				uv: { location: 1, type: vec2(f32) },
			},
			varyings: { vUv: { location: 0, type: vec2(f32) } },
		},
		(ctx, builtins, inputs) => {
			const bounds = uniforms.$("bounds");
			const resolution = uniforms.$("resolution");
			const pixelX = ctx.val(
				"pixelX",
				bounds.x.add(inputs.position.x.mul(bounds.z))
			);
			const pixelY = ctx.val(
				"pixelY",
				bounds.y.add(inputs.position.y.mul(bounds.w))
			);

			const ndcX = ctx.val("ndcX", pixelX.div(resolution.x).mul(_2).sub(_1));
			const ndcY = ctx.val("ndcY", _1.sub(pixelY.div(resolution.y).mul(_2)));

			return {
				position: makeVec4(f32, ndcX, ndcY, 0.0, 1.0),
				varyings: { vUv: { location: 0, value: inputs.uv } },
			};
		}
	);
	builder.fragment(
		...
		(ctx, _builtins, inputs) => {
			...
			const size = ctx.let(
				"size",
				vec2(f32),
				makeVec2(f32, bounds.z, bounds.w)
			);
			...
			// 抗锯齿
			const pixelSize = ctx.val(
				"pixelSize",
				_1.div(ctx.builtins.max(size.x, size.y))
			);
			const alpha = ctx.val(
				"alpha",
